...
do_subset_second_half = True

do_plot                   = True
do_plot_bestC_traces      = True
do_plot_individual_traces = True
...
## 2. SPLIT THE DATAFRAME IN HALF: ONE TO OPTIMIZE C, ONE TO USE THE OPTIMIZED VALUE OF C
...
if do_plot:
        ...
        with TimedBlock("plot individual traces."):
            for pop in pop_sizes:
                for task in ["identity", "category"]:
                    ...
                    plt.legend()
                    file_name = "{}_{}_{}classes_timecourse.pdf".format(pop_names[pop], task, pop_sizes[pop])
                    plt.savefig(file_name, bbox_inches="tight")
                    plt.close("all")
                    print("Wrote {}".format(file_name))

    if do_plot_bestC_traces:
            ...
            for pop in pop_sizes:
                for task in ["identity", "category"]:
                    ...
                    for shuf in [0,1]:
                        ...
                        if not shuf:
                            plt.legend()
                    file_name = "{}_{}_C_timecourse.pdf".format(pop_names[pop], task)
                    plt.savefig(file_name, bbox_inches="tight")
                    plt.close("all")
                    print("Wrote {}".format(file_name))
        
    if do_plot_overlayed_traces:
        with TimedBlock("plot overlayed traces."):
            for task in ["identity", "category"]:
                ...
                plt.legend()
                file_name = "overlayed_{}_timecourse.pdf".format(task)
                plt.savefig(file_name, bbox_inches="tight")
                plt.close("all")
                print("Wrote {}".format(file_name))
    ...
    if do_plot_performance:
        with TimedBlock("plot performance"):
            ...
            file_name = "id_vs_cat{}.pdf".format("_w_shuff" if use_shuffle else "")
            plt.tight_layout()
            plt.savefig(file_name, bbox_inches="tight")
            plt.close("all")
